...
public class TileEntityGoldChest extends TileEntityLockableLoot implements ITickable
{
    ...
    {
        return this.hasCustomName() ? this.customName : "container.gold_chest";
    }
    ...
    {
        return new ContainerGoldChest(playerInventory, this, playerIn);
    }

    @Override
    public String getGuiID()
    {
        return UraMod.MODID + ":gold_chest";
    }
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerGoldChest)
                {
                    if (((ContainerGoldChest)entityplayer.openContainer).getChestInventory() == this)
                    {
