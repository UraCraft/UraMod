...
public class TileEntityStoneChest extends TileEntityLockableLoot implements ITickable
{
    ...
    {
        return new ContainerStoneChest(playerInventory, this, playerIn);
    }

    @Override
    public String getGuiID()
    {
        return UraMod.MODID + ":stone_chest";
    }
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerStoneChest)
                {
                    if (((ContainerStoneChest)entityplayer.openContainer).getChestInventory() == this)
                    {
