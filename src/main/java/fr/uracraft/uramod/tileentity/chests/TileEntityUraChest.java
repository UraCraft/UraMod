package fr.uracraft.uramod.tileentity.chests;

import fr.uracraft.uramod.UraMod;
import fr.uracraft.uramod.container.chests.ContainerUraChest;
...
public class TileEntityUraChest extends TileEntityLockableLoot implements ITickable
{
    ...
    {
        return this.hasCustomName() ? this.customName : "container.ura_chest";
    }
    ...
    {
        return new ContainerUraChest(playerInventory, this, playerIn);
    }

    @Override
    public String getGuiID()
    {
        return UraMod.MODID + ":ura_chest";
    }
    ...
    public void update()
    {
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerUraChest)
                {
                    if (((ContainerUraChest)entityplayer.openContainer).getChestInventory() == this)
                    {
